...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  saveProfile(): void {
    if (this.editForm.valid) {
      ...
      this.userService.updateUserProfile(updatedProfile).pipe(
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      this.isLoading = true;
      this.userService.uploadProfilePicture(input.files[0]).pipe(
        takeUntil(this.unsubscribe$)
      ).subscribe(
        () => {
  ...
  removeFavoriteBeer(beerId: string): void {
    this.userService.removeFavoriteBeer(beerId).pipe(
      takeUntil(this.unsubscribe$)
    ).subscribe(
      () => {
