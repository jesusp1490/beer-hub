      ...
      switchMap((user) => {
        if (user) {
          return combineLatest([
            ...
            this.firestore
              .collection(`users/${user.uid}/ratings`)
              .valueChanges({ idField: "id" }),
            ...
            map(([newRatings, allBeers]) => {
              const oldRatings = allBeers.filter(
                (beer) => beer.rating && typeof beer.rating === "object" && user.uid in beer.rating,
              )
              const combinedRatings = [
                ...newRatings.map((rating: any) => ({
                  id: rating.id,
                ...
                ...oldRatings.map((beer) => ({
                  id: beer.id,
                  rating:
                    beer.rating && typeof beer.rating === "object" && user.uid in beer.rating
                      ? beer.rating[user.uid]
                      : null,
                  ratedAt: null, // We don't have timestamp for old ratings
                })),
              ]
